...
SELECT
	...
	t1.team AS team_name,
	t1.ballpark AS ballpark,
	t2.wins AS last_year_wins,
	DENSE_RANK() OVER(PARTITION BY t2.year ORDER BY t2.wins DESC) AS win_rank,
	t2.playoffs AS last_year_po,
	t2.world_series AS last_year_ws,
	t1.attendance AS attendance
FROM att as t1
INNER JOIN att as t2
ON t1.year-1 = t2.year AND t1.team = t2.team
WHERE t1.team ILIKE '%white%'
...
SELECT *
from teams
WHERE name iLIKE '%white%' AND yearid = 1990
